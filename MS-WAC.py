...
class MS-WAC(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=5, stride=1, bias=True, wt_levels=2, wt_type='db2'):
        super(WTConv2d_SE, self).__init__()
        ...
        self.se_blocks = nn.ModuleList([
            nn.Sequential(
                SEBlock(in_channels * 4),          # 通道注意力
                SpatialAttention()                 # 空间注意力
            ) for _ in range(self.wt_levels)
        ])
        self.fusion_conv = nn.Conv2d(in_channels*(wt_levels+1), out_channels, 1)
        if self.stride > 1:
    ...
    def forward(self, x):
        ...
        for i in range(self.wt_levels):
            ...
            curr_x_tag = self.se_blocks[i](curr_x_tag)  # 新增注意力
...
if __name__ == '__main__':
    block = MS-WAC(in_channels=4, out_channels=4)
